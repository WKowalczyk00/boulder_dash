...
const destroyAround = (i: number, j: number, type: string) => {
    ...
    audio.play();
    if (SETTINGS.board[i][j] != "w" && SETTINGS.board[i][j] != "b")
        SETTINGS.board[i][j] = type
    if (SETTINGS.board[i + 1][j + 1] != "w" && SETTINGS.board[i + 1][j + 1] != "b")
        SETTINGS.board[i + 1][j + 1] = type
    if (SETTINGS.board[i + 1][j - 1] != "w" && SETTINGS.board[i + 1][j - 1] != "b")
        SETTINGS.board[i + 1][j - 1] = type
    if (SETTINGS.board[i+1][j] != "w" && SETTINGS.board[i+1][j] != "b")
        SETTINGS.board[i + 1][j] = type
    if (SETTINGS.board[i][j+1] != "w" && SETTINGS.board[i][j+1] != "b")
